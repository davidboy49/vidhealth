...
CREATE TABLE IF NOT EXISTS action_plans (
    ...
    -- JSON list of strings: the metrics/thresholds this plan wants tracked
    -- before the next one, so the next plan's prompt can read them back in.
    watchlist_json  TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_action_plans_period_end ON action_plans (period_end);
"""
...
# ---------- ACTION PLANS (AI COACH) ----------

def save_action_plan(
    period_start: str, period_end: str, model: str, headline: str,
    plan_text: str, watchlist: list[str] | None = None,
) -> int:
    """
    Saves a generated action plan and returns its new row ID.
    watchlist is stored as a JSON list of strings so the next period's plan
    can read back what this one asked to be tracked.
    """
    init_db()
    now_iso = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    with _conn() as conn:
        with conn.cursor() as cursor:
            cursor.execute("""
            INSERT INTO action_plans (period_start, period_end, generated_at, model, headline, plan_text, watchlist_json)
...
def get_latest_action_plan() -> dict | None:
    """Returns the most recently generated action plan, or None if none exist yet."""
    rows = _fetch_dicts("""
    ...
    plan = rows[0]
    try:
        plan["watchlist"] = json.loads(plan.get("watchlist_json") or "[]")
    except (TypeError, ValueError):
        plan["watchlist"] = []
    return plan


def get_action_plans(limit: int = 10) -> list[dict]:
    """Returns recent action plans, newest first, for a plan-history view."""
    rows = _fetch_dicts("""
        SELECT * FROM action_plans
        ORDER BY generated_at DESC, id DESC
        LIMIT %s
